...
- Add the development `build_it` JSON generator (`build_it:json_dev`) that will generate a file with the `.dart` extension with JSON objects (generated by `json_serializable`) without the `part ` and `part of` directives (and without the files with extensions `.g.dart`). This will be done by combining the parts into one file. This generator allows you to create JSON objects that will not be deleted during build process and will always be available.  
- Add parameter `withDefaults` to the `build_it:json` generator, which will generate null-safe JSON objects by automatically substituting non-null (default) values for non-nullable fields at the generation stage.  
